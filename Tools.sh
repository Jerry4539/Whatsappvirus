#!/bin/bash
# version 2.1

clear


echo ""
echo -e "SUBSCRIBE MY CHANNEL"
echo ""

echo -e "\e[101m Press Enter \e[0m"
read a1

echo "Loading..." |
sleep 2
clear

toilet -f future "Jerry4539 "
echo "-------------------------------------------------------"
echo   "Author   : Jerry4539 $white"
echo   "INSTAGRAM  :  Xhacking_official $white"
echo   "YouTube  : Anomoyous $white"
echo   "Blog     : https://github.com/Jerry4539 $white"
echo "-------------------------------------------------------"
echo "                           v2.0              " |
###################################################
# CTRL + C
###################################################
trap ctrl_c INT
ctrl_c() {
clear
echo  "Detected, Trying To Exit 🚪 ... "
echo ""
echo  "⚠️NOT FOR ILLEGEL USE⚠️"
sleep 1
echo ""
echo "Jerry4539" 
echo ""
echo "BYE BYE 👋👋..." 
echo ""
echo "COME BACK 🔜" 
sleep 1
exit
}

lagi=01
while [ $lagi -lt 20 ];
do
echo ""
echo -e "\e[1;101m\e[1;97m01\e[1;101m\e[0m\e[1;96m TOOL-X\e[0m\n";
echo -e "\e[1;101m\e[1;97m02\e[1;101m\e[0m\e[1;96m LUCIFER\e[0m\n";
echo -e "\e[1;101m\e[1;97m02\e[1;101m\e[0m\e[1;96m GMAILBRUTERV2\e[0m\n";
echo -e "\e[1;101m\e[1;97m03\e[1;101m\e[0m\e[1;96m WEEMAN\e[0m\n";
echo -e "\e[1;101m\e[1;97m03\e[1;101m\e[0m\e[1;96m PHISHER\e[0m\n";
echo -e "\e[1;101m\e[1;97m04\e[1;101m\e[0m\e[1;96m RED_HAWK\e[0m\n";
echo -e "\e[1;101m\e[1;97m05\e[1;101m\e[0m\e[1;96m TECH-HACK\e[0m\n";
echo -e "\e[1;101m\e[1;97m06\e[1;101m\e[0m\e[1;96m BRUTEFORCE_FACEBOOK\e[0m\n";
echo -e "\e[1;101m\e[1;97m07\e[1;101m\e[0m\e[1;96m BOT_KONEN_FACEBOOK\e[0m\n";
echo -e "\e[1;101m\e[1;97m08\e[1;101m\e[0m\e[1;96m TERMUX-BASIC-PACKAGE\e[0m\n";
echo -e "\e[1;101m\e[1;97m09\e[1;101m\e[0m\e[1;96m OSIF\e[0m\n";
echo -e "\e[1;101m\e[1;97m09\e[1;101m\e[0m\e[1;96m Brute-Forc-Twitter\e[0m\n";
echo -e "\e[1;101m\e[1;97m10\e[1;101m\e[0m\e[1;96m INSTAGRAM-BRUTEFORCE\e[0m\n";
echo -e "\e[1;101m\e[1;97m11\e[1;101m\e[0m\e[1;96m FIRECRACK\e[0m\n";
echo -e "\e[1;101m\e[1;97m12\e[1;101m\e[0m\e[1;96m EVIL-EYE\e[0m\n";
echo -e "\e[1;101m\e[1;97m13\e[1;101m\e[0m\e[1;96m TECH-CRASH\e[0m\n";
echo -e "\e[1;101m\e[1;97m14\e[1;101m\e[0m\e[1;96m SPEED-X\e[0m\n";
echo -e "\e[1;101m\e[1;97m15\e[1;101m\e[0m\e[1;96m CAMPHISH\e[0m\n";
echo -e "\e[1;101m\e[1;97m00\e[1;101m\e[0m\e[1;96m SOCIALBOX\e[0m\n";
echo -e "\e[1;101m\e[1;97m02\e[1;101m\e[0m\e[1;96m YOUTUBE-SUB-LIKES\e[0m\n";
echo -e "\e[1;101m\e[1;97m00\e[1;101m\e[0m\e[1;96m EXIT\e[0m\n";
echo ""
echo  "╭─Jerry4539"
read -p "╰──►" pil;


#TOOL-X


case $pil in
01) clear
toilet -f standard " Jerry4539 " -F gay
git clone https://github.com/Rajkumrdusad/Tool-X
cd Tool-X
sh install.aex
 

;;


#LUCIFER


02) clear
toilet -f standard " Jerry4539 " -F gay
git clone https://github.com/rixon-cochi/Lucifer.git
cd Lucifer
chmod +x *
bash setup.sh
ls
bash instacracker.sh


;;


#GMAILBRUTERV2


03) clear
toilet -f standard " Jerry4539 "
git clone https://github.com/DEMON1A/GmailBruterV2
cd GmailBruterV2
python3 GmailBruterV2.py


;;


#WEEMAN	


04) clear
toilet -f standard " Jerry4539 " -F gay
git clone https://github.com/evait-security/weeman.git
cd weeman
python2 weeman.py


;;


#PHISHER


05) clear
toilet -f standard " Jerry4539 " -F gay
git clone https://github.com/yezz123/Phisher
cd Phisher
bash Phisher.sh


;;


#RED_HAWK


06) clear
toilet -f standard " Jerry4539 " -F gay
git clone https://github.com/Tuhinshubhra/RED_HAWK
cd RED_HAWK
php rhawk.php


;;


#TECH-HACK


07) clear
toilet -f standard " Jerry4539 " -F gay
git clone https://github.com/rixon-cochi/tech-hack.git
cd tech-hack
chmod +x *
unzip sites.zip
bash cracker.sh


;;


#BRUTEFORCE_FACEBOOK


08) clear
toilet -f standard " Jerry4539 " -F gay
git clone https://github.com/TUANB4DUT/FACEBOOK-BRUTEFORCE 
cd FACEBOOK-BRUTEFORCE
python2 bruteforce.py


;;


#BOT_KONEN_FACEBOOK


09) clear
toilet -f standard " Jerry4539 " -F gay
pip2 install mechanize
git clone https://github.com/Senitopeng/Botkomena.git
cd Botkomena
python2 botkomena.py


;;


#TERMUX-BASIC-PACKAGE


10) clear
toilet -f standard " Jerry4539 " -F gay
https://github.com/rixon-cochi/basic-pkg
cd basic-pkg
bash basic.sh


;;


#OSIF


11) clear
toilet -f standard " Jerry4539 " -F gay
git clone https://github.com/ciku370/OSIF
cd OSIF
pip2 install -r requirements.txt
python2 osif.py


;;


#Brute-Forc-Twitter


12) clear
toilet -f standard " Jerry4539 "
git clone https://github.com/0xfff0800/Brute-Forc-Twitter
pip3 install mechanize
pip3 install proxylist
pip3 install argparse
cd Brute-Forc-Twitter
chmod +x Brute-Forc-Twitter.py
python3 Brute-Forc-Twitter.py


;;


#INSTAGRAM-BRUTEFORCE


13) clear
toilet -f standard " Jerry4539 " -F gay
git clone https://github.com/linuxkukeren/instagram
pip install requests
pip install -r requirement.txt
pip2 install -r requirement.txt
python2 linuxkukerenhacking.py


;;


#FIRECRACK


14) clear
toilet -f standard " Jerry4539 " -F gay
git clone https://github.com/Ranginang67/Firecrack
cd Firecrack
pip2 install -r requirements.txt
python2 firecrack.py


;;


#EVIL-EYE


15) clear
toilet -f standard " @Jerry4539 " -F gay
git clone https://github.com/rixon-cochi/Evil-eye
Cd Evil-eye
Chmod +x *
bash requirement.sh
bash Evil-l.sh


;;


#TECH-CRASH


16) clear
toilet -f standard " @Jerry4539 " -F gay
git clone https://github.com/rixon-cochi/techcrash
cd techcrash
php Whatsbot.php


;;


#SPEED-X


17) clear
toilet -f standard " @Jerry4539 " -F gay
git clone https://github.com/TheSpeedX/TBomb.git
cd TBomb
chmod +x TBomb.sh
./TBomb.sh


;;


#CAMPHISH


18) clear
toilet -f standard " @Jerry4539 " -F gay
git clone https://github.com/techchipnet/CamPhish
cd CamPhish
bash camphish.sh


;;


#SOCIALBOX


19) clear
toilet -f standard " @Jerry4539 " -F gay
git clone https://github.com/TunisianEagles/SocialBox.git
cd SocialBox
chmod +x SocialBox.sh
chmod +x install-sb.sh
./install-sb.sh
./SocialBox.sh


;;


#YOUTUBE-SUB-LIKES


20) clear
toilet -f standard " @jerry4539 " -F gay
pkg install nano
git clone https://github.com/Jerry4539/Sub-Like
python2 hakyt.py


;;


00) echo "Bye, this tool was created by : @Jerry4539 "
exit


;;


*) echo "sorry, the  option you looking is not found"
esac
done
done

=================="
